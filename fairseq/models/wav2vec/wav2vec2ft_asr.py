...
@dataclass
class Wav2Vec2FTASRConfig(FairseqDataclass):
    ...
    n_freeze_layers: int = field(
        default=0, metadata={"help": "number of encoder layers to freeze"}
    )
    n_remove_layers: int = field(
        default=0, metadata={"help": "number of encoder layers to remove"}
    )
    gradient_checkpoint: ChoiceEnum(['True', 'False']) = field(
        default='False', metadata={"help": "If true, use gradient checkpoint to save memory"}
    )
    ...
    attention_type: str = field(
        default="conditional-full:improved-clustered",
        metadata={"help": "attention type to be used default:conditional-full:improved-clustered"},
    )
    clusters: int = field(
        default=200,
        metadata={"help": "number of clusters default:200"},
    )
    topk: int = field(
        default=32,
        metadata={"help": "number of keys for improved-clustered default:32"},
    )
    length_limit: int = field(
        default=512,
        metadata={"help": "use full attention below this sequence length default:512"},
    )
    iterations: int = field(
        default=10,
        metadata={"help": "number of lloyd iterations recommended [kmeans:2, lsh:10]"},
    )
    bits: int = field(
        default=63,
        metadata={"help": "number of bits to lsh clustering default:63"},
    )
    hash_bias: bool = field(
        default=True,
        metadata={"help": "use hash bias to l2-lsh clustering default:True"},
    )
    

@register_model("wav2vec2ft_asr", dataclass=Wav2Vec2FTASRConfig)
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        builder = AttentionBuilder.from_kwargs(
            ...
            query_dimensions=args.encoder_embed_dim / args.encoder_attention_heads,
        )
        ...
        self.gradient_checkpoint = strtobool(args.gradient_checkpoint)
        self.n_freeze = args.n_freeze_layers
        self.n_remove = args.n_remove_layers
        self.n_active = args.encoder_layers - args.n_remove_layers
        for i in range(self.n_freeze):
            for p in self.layers[i].parameters():
                p.requires_grad = False

        for i in range(self.n_remove):
            for p in self.layers[-(i+1)].parameters():
                p.requires_grad = False
        ...
        if self.n_freeze > 0:
            for param in self.pos_conv.parameters():
                param.requires_grad = False
